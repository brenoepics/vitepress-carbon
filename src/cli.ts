...
const { cyan, bold, yellow, green, red } = picocolors
const argv = minimist(process.argv.slice(2))

const logVersion = (logger = console) => {
  logger.info(`\n  ${green(`${bold('vitepress-carbon')}`)}\n`)
}
...
enum ScaffoldThemeType {
  ...
  DefaultCustom = 'carbon theme + customization'
}
          ...
          options: [
            {
              ...
              label: 'Carbon Theme',
              hint: 'Streamlined Theme, good-looking docs'
            },
            {
              // @ts-ignore
...
function scaffold({
  ...
  if (injectNpmScripts) {
    ...
    const execCommand = getPackageManger()
