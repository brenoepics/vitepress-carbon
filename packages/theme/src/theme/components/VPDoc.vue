...
.content-file {
  ...
  min-width: 0;
  max-width: 100%;
  /* Clip rather than let a long title escape `.content-container` (#28). */
  overflow: hidden;
}
...
/* Pull the breadcrumb's hover padding back so its label optically aligns
   with the prose column below. Only the title link clips — the sibling action
   items host absolutely positioned menus (VPLlmsPageActions) that have to
   escape their box. */
...
.content-box-text {
  font-weight: 600;
  /* Without this the span refuses to shrink below its content width, so the
     ellipsis below never kicks in (#28). The full title stays available via
